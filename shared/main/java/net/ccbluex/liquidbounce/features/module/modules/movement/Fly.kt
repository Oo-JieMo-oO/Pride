...
class Fly : Module() {
    ...
    fun onBlockBB(event: BlockBBEvent) {
        when (modeValue.get().toLowerCase()) {
            "fakeground" -> {
                ClientUtils.displayChatMessage(event.block.unwrap().toString())
                if (mc2.world.isAirBlock(BlockPos(mc.thePlayer!!.posX, mc.thePlayer!!.posY - 1, mc.thePlayer!!.posZ)) && event.y <= launchY) {
                    ClientUtils.displayChatMessage("你好")
                    event.boundingBox = classProvider.createAxisAlignedBB(event.x.toDouble(), event .y.toDouble(), event.z.toDouble(), event.x + 1.0, launchY   , event.z + 1.0)
                }
            }
        }
    }
    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        run {
            when (modeValue.get().toLowerCase()) {
                ...
                "hyt4v4test" -> {
                    if (mc.thePlayer!!.posY <= hytStartY) {
